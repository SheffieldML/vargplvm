...
if ~exist('showPlots'), showPlots=false; end
if ~exist('saveFigures'), saveFigures=false; end
if ~exist('numberTestPoints'), numberTestPoints = -1; end
% Set to 1 to test on the training data itself, set to 0 to use the test
% dataset.
if ~exist('testOnTraining'), testOnTraining = 0; end
if ~exist('resultsDynamic'),  resultsDynamic = 0; end
if ~exist('showVideo'), showVideo = 0; end
if ~exist('reconstrIters'), reconstrIters = 4500; end

if ~exist('experimentNo')
    if dynUsed
        experimentNo = 120;
    else
        experimentNo = 311;
...
load(['demHumanPoseSvargplvm' num2str(experimentNo)]);
if dynUsed
...
if exist('testReoptimise') && testReoptimise, model.dynamics.reoptimise=testReoptimise; end
...
if dynUsed
    ...
    for i=1:size(Z_test,1)
        ...
        dst = dist2(Yts{obsMod}(i,:), Ytr{obsMod});
        [mind, mini(i)] = min(dst);
    end
    ...
    iters = reconstrIters;
    % Do reconstruction in test data
    [x, varx] = vargplvmOptimiseSeqDyn(model.comp{obsMod}, vardistx, Yts{obsMod}, 1, iters);
    ...
    [x_star, varx_star, modelUpdated] = vargplvmDynamicsUpdateModelTestVar(model.comp{obsMod}, barmu, lambda, Yts{obsMod});
    modelOrig = model;
...
for i=1:length(testInd)
    curInd = testInd(i);
    if testOnTraining
        y_star = Ytr{obsMod}(curInd,:);
        z_star = Ytr{infMod}(curInd,:);
        sil_star = Yim(curInd,:);
...
if dynUsed
    meanPose = repmat(mean(Ytr{infMod}),size(Z_test,1),1);
    errors.meanPose = xyzankurError(meanPose, Z_test);
    errors.NNYspace = xyzankurError(Ytr{infMod}(miniNNY,:), Z_test);
    % errors.NNYspace = xyzankurError(Ytr{2}(mini,:), Z_test)
    errors.NNXspace = xyzankurError(Ytr{infMod}(indsAllOrig,:),Z_test);
