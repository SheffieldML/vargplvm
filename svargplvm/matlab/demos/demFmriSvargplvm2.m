...
if ~exist('itNo')         ,  itNo = [800 1500 500 500];              end     % Default: 2000
if ~exist('indPoints')    ,  indPoints = 80;          end     % Default: 49
if ~exist('latentDims')    ,  latentDims = 16;          end
...
% Initialise latent space by applying @initX separately and then
...
if ~exist('dataType'), dataType = 'fmri'; end
...
if strcmp([activity{1}{:}], [activity{2}{:}]) ~= 1
...
switch initial_X
    ...
    case 'together'
        X_init = initFunc([m{1} m{2}], latentDims);
end
...
for i=1:length(testInd)
    curInd = testInd(i);
    if testOnTraining
        currentLabel{i} = activity{obsMod}(curInd);
    ...
    for k=1:length(ind)
        ...
        indicesX{i} = [indicesX{i} ',' activity{obsMod}(ind(k))];
