% NEWDEMYALESVARGPLVM3 Run the Shared Var. GP-LVM on a subset of the Yale
% faces.
...
for i=1:size(Yall{1},1)
    for d=1:numberOfDatasets
        ...
        imagesc(reshape(Yall{d}(i,:),height, width)),colormap('gray')% title(num2str(identities{d}(i))),colormap('gray');
        axis off
    end
    if i==1
        pause
...
for i=1:numberOfDatasets
    ...
    model{i}.vardist.covars = 0.5*ones(size(model{i}.vardist.covars)) + 0.001*randn(size(model{i}.vardist.covars));
     ...
     if model{1}.DgtN
             model{i}.beta=1/(0.01*var(model{i}.mOrig{i}(:)));
     else
