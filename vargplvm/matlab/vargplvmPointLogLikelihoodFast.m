function [ll, model] = vargplvmPointLogLikelihoodFast(model, vardistx, y)
...
indexMissing = model.testPrecomp.indexMissing;
...
if dynUsed == 1
   ...
   TrPP = model.testPrecomp.TrPP; %sum(sum(P .* P));
end 
...
if ~isempty(indexMissing)
   ...
   TrYY = model.testPrecomp.TrYY; %sum(sum(mOrig(:,indexMissing) .* mOrig(:,indexMissing)));
...
model.TrYY = model.testPrecomp.TrYY2;
model.C = model.invLm * model.Psi2 * model.invLmT;
model.TrC = sum(diag(model.C)); % Tr(C)
model.At = (1/model.beta) * eye(size(model.C,1)) + model.C; % At = beta^{-1} I + C
model.Lat = jitChol(model.At)';
...
if isfield(model, 'DgtN') && model.DgtN
    model.P = model.P1 * (model.Psi1' * model.testPrecomp.mReduced);
else
    model.P = model.P1 * (model.Psi1' *  [model.m(:, indexPresent); y]);
end
