% DEMYALESVARGPLVM2OLD Run the Shared Var. GP-LVM on a subset of the Yale
% faces.
...
selDirs = '04';
    ...
    dirFrom=[baseDir filesep 'yaleB' selDirs];
    a=dir(dirFrom);
    counter = 0;
    for i=1:length(a)
        if length(a(i).name)>4 & strcmp(a(i).name(end-2:end),'pgm') ...
            ...
            counter = counter+1;
            coords(counter,1) = str2num(a(i).name(end-11:end-8));
            coords(counter,2) = str2num(a(i).name(end-6:end-4));
        end
    end
%}
...
load YaleCoordinates

N1 = size(Y{1},1);
Yall{2} = [Y{4};Y{5};Y{6}];
coordsAll = [coords ; coords; coords];
...
for i=1:size(Yall{1},1)
    for d=1:numberOfDatasets
        ...
        imagesc(reshape(Yall{d}(i,:),height, width)),colormap('gray'), title([num2str(coordsAll(i,1)) ' ' num2str(coordsAll(i,2))]) % title(num2str(identities{d}(i))),
    end
    pause
end
%}
...
% % test coordinates
% for i=1:size(Yall{1},1) 
%     imagesc(reshape(Yall{2}(i,:),height, width)), title([num2str(coords(i,1)) ' ' num2str(coords(i,2))]),colormap('gray');
%     pause
% end
